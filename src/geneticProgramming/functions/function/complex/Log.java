...
public class Log extends UnaryNode
{
    ...
    public double evaluate(double[] programParameters) {
        return Math.log(this.node.evaluate(programParameters));
    }
    ...
    public Node simplify() {
        ...
        if (simplifiedNode instanceof Constant) {
            return new Constant(Math.log(simplifiedNode.evaluate(NO_ARGS)));
        } else if (simplifiedNode != this.node) {
            return new Log(simplifiedNode);
        } else {
            return this;
        }
